...
Class tom_upload{
	...
	function init($attach, $type = 'tomwx', $extid = 0, $forcename = '') {
	...
	function get_image_info($target, $allowswf = false) {
		$ext = tom_upload::fileext($target);
		$isimage = tom_upload::is_image_ext($ext);
	...
	function get_target_extension($ext) {
		static $safeext  = array('attach', 'jpg', 'jpeg', 'gif', 'png', 'swf', 'bmp', 'txt', 'zip', 'rar', 'mp3','csv');
	...
	function get_target_dir($type, $extid = '', $check_exists = true) {
		...
		if($type == 'album' || $type == 'forum' || $type == 'portal' || $type == 'category' || $type == 'profile' || $type == 'tomwx') {
			$subdir1 = date('Ym');
			$subdir2 = date('d');
			$subdir = $subdir1.'/'.$subdir2.'/';
		...
		$check_exists && tom_upload::check_dir_exists($type, $subdir1, $subdir2);

		return $subdir;
	}

	function check_dir_type($type) {
		return !in_array($type, array('forum', 'group', 'album', 'portal', 'common', 'temp', 'category', 'profile','tomwx')) ? 'temp' : $type;
	}

	function check_dir_exists($type = '', $sub1 = '', $sub2 = '') {

		$type = tom_upload::check_dir_type($type);
		...
		if(!$res) {
			$res = $typedir && tom_upload::make_dir($typedir);
			$res && $subdir1 && ($res = tom_upload::make_dir($subdir1));
			$res && $subdir1 && $subdir2 && ($res = tom_upload::make_dir($subdir2));
		}
	...
	function save_to_local($source, $target) {
		if(!tom_upload::is_upload_file($source)) {
			$succeed = false;
